...
class Pairs2 rep where
  ...
  fst'     :: rep (sa, sb) (da, db)    -> rep sa da
  snd'     :: rep (sa, sb) (da, db)    -> rep sb db

class Equality2 rep where
  eq'      :: Eq a => rep a a      -> rep a a               -> rep Bool Bool

class Plus2 rep where
  left'    :: rep a a               -> rep (Either a b) (Either a b)
  right'   :: rep b b               -> rep (Either a b) (Either a b)
  either'  :: rep (a -> c) (a -> c) -> rep (b -> c) (b -> c) -> rep (Either a b) (Either a b)  -> rep c c
...
liftP1 l f _ (P _ (Just x)) = P (l . f $ x) (Just . f $ x)
